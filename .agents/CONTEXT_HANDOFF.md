...
## 2. Standardized Experiment Subfolder Hierarchy (`results/`)
All experiments are saved in clean dedicated subfolders per run, each containing `rollout.mp4`, `trajectory_filmstrip.png`, `motion_heatmap.png`, `metadata.json`, and `data.npz`:
1. `results/epic_ecosystem/`: Master 5-minute HD broadcast video (22,500 steps, 7,500 frames, 56.6 MB MP4) with 8 species interacting around 4 DodgerBlue arena obstacle pillars.
2. `results/baseline_imgep/`: Top-3 IMGEP curiosity search elites (`elite_1/`, `elite_2/`, `elite_3/`) from 40 goal exploration trials.
3. `results/wall_obstacles/`: Top-3 IMGEP elites (`elite_1/`, `elite_2/`, `elite_3/`) navigating around DodgerBlue barrier walls.
4. `results/barrier_constriction/`: Constriction sweep (`width_08/`, `width_16/`, `width_24/`, `width_32/`) with active soliton-wall deformation and transmission plot `transmission_curve.png`.
5. `results/resource_depletion/`: Static nutrient baseline vs dynamic foraging depletion comparison (`static_baseline/`, `dynamic_depletion/`).
6. `results/showcase/`: Physical mechanisms (`method_1_gene_mutation/` and `method_2_negotiation_rule/`).
7. `results/scaleup/`: Scaled-up runs on $512 \times 512$ canvas (`rerun_1/`, `rerun_2/`) with FPS selection from IMGEP archive.
8. `results/orbium/`: Classic *Orbium unicaudatus* glider physics verification video.
9. `results/agentic_loop/`: Multi-generation autonomous AI Scientist discovery state.
