...
10. [Step-by-Step Reproduction Guide](#10-step-by-step-reproduction-guide)
    - 10.1 Unified CLI Experiment Suite
    - 10.2 Hardware Portability & Computational Scaling
    - 10.3 Containerization & Docker Execution
...
## 1B. Flow-Lenia Physics Authenticity Classification

This section documents the exact degree to which each thesis experiment uses canonical Flow-Lenia physics, distinguishing between fully authentic PDE dynamics, acceptable hybrid extensions, and deliberate approximations.

### Authenticity Criteria

An experiment is classified as **Fully Authentic** if it uses all four canonical components:
1. FFT multi-shell ring-kernel convolution: $U_k = \text{irfft2}(\hat{m} \cdot \hat{K}_k)$
2. Canonical growth mapping: $G_k = 2\exp\!\left(-\tfrac{(U_k - \mu)^2}{2\sigma^2}\right) - 1$
3. Moroz bilinear reintegration advection (exact mass conservation)
4. Emergent motility from $\mathbf{v} = v_s[(1-\alpha)\nabla G - \alpha\nabla U]$ — no external force terms

### Classification Table

| Experiment | FFT+Moroz | External Forces | Gene-Mixing | Classification |
| :--- | :---: | :---: | :---: | :---: |
| `run_barrier_constriction.py` | ✅ | None | ❌ | **Fully Authentic** |
| `run_resource_depletion.py` | ✅ | None | ✅ Gene-wise | **Fully Authentic (most canonical)** |
| `run_epic_ecosystem.py` | ✅ | Minimal scent | ❌ | **Substantially Authentic** |
| `run_chemotaxis_calibration.py` | ✅ | $\chi\nabla\text{scent}$ (Michel 2025) | ❌ | **Hybrid (accepted extension)** |
| `run_topological_transport.py` | ✅ | $\chi\nabla\text{geodesic}$ | ❌ | **Hybrid** |
| `run_collective_bridge.py` | ✅ | Fixed direction vector | ❌ | **Hybrid** |
| `run_traveling_salesperson.py` | ✅ | Softmax city field | ❌ | **Hybrid** |
| `run_predator_prey.py` | ❌ rendering only | Discrete active matter | ❌ | **Discrete AM + FL visual morphology** |

### Hybrid Velocity Equation (applies to all Hybrid experiments)

$$\mathbf{v} = \tanh\!\left( v_s \left[(1-\alpha)\nabla G - \alpha\nabla U\right] + \chi \cdot \nabla\phi_{\text{external}} \right)$$

where $\phi_{\text{external}}$ is a scent field, geodesic potential, directional vector, or softmax city field depending on the experiment.

### Critical Demarcation: Mechanical Test Forces vs. External Route Solvers

A crucial scientific distinction must be made regarding how $\nabla\phi_{\text{external}}$ is used across different experiments:

1. **Chemotaxis as a Calibrated Tensile Test Force (`run_barrier_constriction.py` & `run_chemotaxis_calibration.py`)**:
   - In the barrier constriction assay, $\phi_{\text{external}}(\mathbf{x}) \propto x$ is a simple, uniform horizontal ramp.
   - **No navigation problem is being solved**: the gradient simply pulls mass strictly to the right ($+x$).
   - The gradient serves as a **calibrated physical test force** (analogous to gravity in fluid dynamics or a tensile test in materials science). The scientific question is strictly *biomechanical and material*: can the soliton's internal surface tension and elastomeric membrane deform and squeeze through a narrow $8\text{ px}$ aperture without tearing or dissipating?

2. **The "Navigation Breakthrough" Fallacy (`run_topological_transport.py` & `run_traveling_salesperson.py`)**:
   - In a complex 2D labyrinth with dead-ends and bifurcations, a simple straight-line scent gradient $\nabla S$ fails completely: the vector points directly through solid obstacle walls, trapping the soliton against the first interior barrier in a permanent deadlock.
   - To navigate around corners, the scent field must diffuse through open corridors, solving the Laplace / geodesic distance equation ($\nabla^2 \phi = 0$ with $\phi=0$ on walls).
   - **Crucial Epistemological Fact**: It is the **external geodesic wavefront propagation algorithm** that computes the shortest path around obstacle corners, NOT the Flow-Lenia cellular automaton.
   - If a local continuous CA with $R \approx 15\text{ px}$ neighborhood kernels could spontaneously solve global $256\times 256$ labyrinths without memory or global lookahead, that would be an unprecedented computational breakthrough. In reality, Flow-Lenia provides the **continuous soft-matter physical body** (surface tension, droplet cohesion, elastomeric deformation), while the external potential field provides the **navigational intelligence**.
   - This exact distinction is why topological transport and TSP are classified as **Supplementary Exploratory Modules**: to maintain absolute scientific integrity and avoid misleading claims of "emergent cognitive pathfinding".

### Why Predator-Prey Cannot Use the Full PDE

Isolated Flow-Lenia solitons (without evolutionary gene-wise mixing) are **fundamentally unstable**: they lose >97% of their mass within 2,000 steps regardless of parameter tuning. Solitons are only stable within an evolutionary multi-patch gene-mixing context (as in `run_resource_depletion.py`). Two-species field coupling (Lotka-Volterra $G_\text{eff} = G \cdot (1 + \lambda U_\text{other})$) further destabilises the attractor by pushing the convolution signal outside the peak of $G(U)$. This is a mathematical property of the PDE, not a failure of parameter search. The predator-prey experiment therefore uses discrete active matter mechanics with an authentic Flow-Lenia multi-shell soliton visual morphology. See `core/flow_lenia_trophic.py` for the full two-species PDE implementation that was benchmarked and rejected.
...
### 8.6 Supplementary Exploratory Modules (`experiments/supplementary/`)

These modules explore practical applications and test the boundaries of Flow-Lenia by coupling Flow-Lenia PDE physics or visual profiles with external potential fields or discrete active matter mechanics. They are isolated in `experiments/supplementary/` and output to `results/supplementary/`:

#### Supplementary Act S1: Topological Navigation, Dynamic Adaptation & Decentralized Transport Networks
- **Objective**: Study how continuous mass-conserving solitons navigate complex spatial graphs when coupled with geodesic distance potentials ($\chi \nabla \phi_{\text{geodesic}}$).
- **Scenarios**:
  - *Scenario A (Fluid Maze Solver)*: Shortest-path traversal and dead-end retraction via surface tension (`results/supplementary/topological_transport/scenario_maze/`).
  - *Scenario B (Dynamic Obstacle Rerouting)*: Mid-flight gate closure at $t=600$, backpressure U-turn and secondary path selection (`results/supplementary/topological_transport/scenario_dynamic_reroute/`).
  - *Scenario C (Tokyo Rail Archipelago)*: Branching transport network synthesis connecting 4 distributed city hubs (`results/supplementary/topological_transport/scenario_tokyo_rail/`).
  - *Scenario D (Swarm Channeling & Funnel Fusion)*: Dual-droplet convergence through a funnel into a unified super-droplet (`results/supplementary/topological_transport/scenario_swarm_channeling/`).
- **CLI**: `uv run python run_experiment.py --mode topological_transport --scenario all --seeds 42 101 2024`

#### Supplementary Act S2: Predator-Prey Trophic Dynamics — Discrete Active Matter with Flow-Lenia Morphology
- **Objective**: Model multi-trophic predator-prey dynamics with Lotka-Volterra limit cycles and biologically motivated metabolic scaling.
- **Architectural Rationale**: Hybrid discrete active matter with authentic multi-shell soliton visual morphology. (Pure 2-species Flow-Lenia PDE is mathematically unstable for isolated individuals without gene-mixing; see §1B and §11.6).
- **Results**: Stable closed Lotka-Volterra phase-space limit cycles ($60 - 75$ orbits), predator dynamic starvation/swelling ($3.67\times$), herbivore grazing growth ($2.15\times$) (`results/supplementary/predator_prey/`).
- **CLI**: `uv run python run_experiment.py --mode predator_prey --seeds 42 101 2024 --steps 4500`

#### Supplementary Act S3: Traveling Salesperson Problem (TSP) — Softmax Multi-Body Gradient Optimization
- **Objective**: Test continuous Flow-Lenia solitons as a physical substrate for solving the NP-hard 2D Euclidean TSP via temperature-scaled Softmax inverse-distance gravitational attraction ($\tau = 25.0\text{ px}$) and repulsive refractory wakes.
- **Results**: Spontaneous Hamiltonian tour synthesis ($[0, 1, 3, 2, 6, 5, 4, 0]$), achieving $92.2\% - 92.9\%$ tour efficiency relative to the exact mathematical brute-force minimum ($L_{\text{optimal}} = 586.05\text{ px}$) (`results/supplementary/traveling_salesperson/`).
- **CLI**: `uv run python run_experiment.py --mode tsp --seeds 42 101 2024 --steps 4500`

#### Supplementary Act S4: Collective Bridge Building & Swarm Highway Transport
- **Objective**: Model bio-inspired self-assembling living structures where pioneer builder solitons span an impassable chasm, creating a conductive catenary scaffold for continuous forager transport.
- **Results**: $100\%$ gap-spanning living bridge scaffold with steady $35.5\% - 42.2\%$ mass transport onto the foraging plateau (`results/supplementary/collective_bridge/`).
...
The simulation engine is implemented entirely in native JAX without proprietary assembly kernels. All 2D spatial FFT convolutions, Moroz bilinear flux advections, and time integration loops compile to hardware-native kernels via Google XLA.

#### Hardware Resource & Scaling Matrix

| Hardware Tier | Memory / VRAM | Concurrency Model | Practical Scaling & Recommended Budget |
| :--- | :--- | :--- | :--- |
| **Workstation GPU** | $\ge 24\text{ GB}$ (e.g. 24–32 GB) | `vmap` (16–32 parallel trials) | Full throughput; $512\times 512$ canvas; full IMGEP archives. Reference testing on RTX 5090. |
| **Consumer GPU** | $8–16\text{ GB}$ | `vmap` (4–8 parallel trials) | Standard $256\times 256$ grid; keep batch sizes moderate to avoid VRAM limits. |
| **HPC / Cloud Accelerators** | $40–80\text{ GB}$ (e.g. A100/H100) | `vmap` (64+ parallel trials) | High-throughput parallel multi-seed discovery campaigns. |
| **CPU / Metal / ROCm** | Host RAM / Unified | Sequential or thread-pool | Unit tests, CI/CD verification, mathematical debugging. |

#### Memory & VRAM Configuration
To prevent JAX from aggressively claiming all available VRAM at initialization on shared systems or consumer cards:
```bash
# Disable aggressive 90% preallocation:
export XLA_PYTHON_CLIENT_PREALLOCATE=false
# Restrict JAX memory footprint to 80% of device VRAM:
export XLA_PYTHON_CLIENT_MEM_FRACTION=0.80
```

#### HPC SLURM Batch Execution Template
```bash
#!/bin/bash
#SBATCH --job-name=flow_lenia_hpc
...
uv run python run_experiment.py --mode agentic_loop --generations 20 --output_dir results/slurm_agentic
```

### 10.3 Containerization & Docker Execution

Containerized execution ensures complete bitwise reproducibility across disparate Linux distributions, cloud virtual machines, and university clusters without manual dependency management.

```bash
# 1. Build standard container
docker build -t flow-lenia:latest .

# 2. Run automated test suite inside container (CPU mode)
docker run --rm flow-lenia:latest

# 3. Execute GPU-accelerated simulation with host volume mounting:
docker run --gpus all --rm -v $(pwd)/results:/app/results flow-lenia:latest \
    python run_experiment.py --mode barrier_constriction --widths 8 16 24 32

# 4. Apptainer / Singularity execution (for locked HPC environments):
apptainer build flow-lenia.sif docker-daemon://flow-lenia:latest
apptainer run --nv flow-lenia.sif python run_experiment.py --mode orbium
```
...
### 11.6 The Navigation Breakthrough Illusion & Demarcation of Problem-Solving Agency
A recurring skepticism when evaluating continuous artificial life systems is asking: **where does the problem-solving intelligence actually reside?**

1. **The Navigation Breakthrough Illusion**:
   - If a purely local continuous cellular automaton (with neighborhood radius $R \approx 15\text{ px}$) could spontaneously navigate a global $256\times 256$ labyrinth with cul-de-sacs, bifurcations, and obstacles without memory, global perception, or an external potential field, that would constitute a major, unprecedented breakthrough in decentralized algorithmic computing.
   - In reality, Flow-Lenia **cannot** solve global pathfinding on its own. A simple straight-line scent gradient $\nabla S$ causes the soliton to plunge directly into the first interior obstacle wall, becoming hopelessly stuck in a local minimum.
   - To traverse the labyrinth, the system must couple Flow-Lenia with an **external geodesic potential field** $\phi(\mathbf{x})$, precomputed by solving Laplace / Dijkstra wavefront diffusion through the open corridors ($\nabla^2 \phi = 0$ with $\phi=0$ on solid walls).

2. **Demarcation of Roles (Who Does What?)**:
   - **The External Potential Field ($\nabla \phi$)**: Solves the global topological routing problem (path planning around corners and obstacle avoidance).
   - **The Flow-Lenia PDE Engine**: Provides the **soft-matter elastomeric body** (droplet cohesion, surface tension, mass conservation, and fluid deformation through narrow corridors).
   - **Why This Matters for the Thesis**: Demarcating these roles prevents misleading or fraudulent claims of "emergent cognitive intelligence". It reframes Flow-Lenia accurately as an expressive, continuous physical substrate for bio-inspired soft-matter simulation when coupled with appropriate environmental potentials.

3. **The Soliton Population Dilemma (Predator-Prey)**:
   - Modeling isolated predator and prey individuals in a pure two-species Flow-Lenia PDE fails because isolated solitons lose $>97\%$ of their mass over 2,000 steps without continuous multi-patch gene-mixing.
   - *Conclusion*: Flow-Lenia solitons are **inherently collective, dissipative structures** supported by population-level mass exchange, not isolated classical particles. Multi-trophic ecologies require either a collective multi-patch substrate or a hybrid active-matter framework.
