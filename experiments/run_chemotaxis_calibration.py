...
ARCHITECTURAL NOTE — Hybrid Flow-Lenia:
  This experiment uses *authentic* Flow-Lenia physics for morphological cohesion (FFT multi-shell
  ring-kernel convolution, canonical growth mapping G(U), Moroz bilinear reintegration advection),
  plus a **chemotactic extension term** (chi * ∇scent) added to the velocity field.

  The velocity equation is:
    v = tanh( v_scale * [(1-α)∇G - α∇U] + χ * ∇scent )

  The chi*∇scent term is NOT part of the original Plantec et al. (2025) Flow-Lenia PDE. It is a
  Flow-Lenia chemotaxis extension described in Michel et al. (2025, arXiv:2505.15998) §3.2,
  where an external chemical scent gradient provides directional bias to the otherwise isotropic
  soliton motility. The soliton morphology, mass conservation and self-organisation are fully
  governed by the Flow-Lenia PDE; only the *direction of travel* is externally biased.
"""
