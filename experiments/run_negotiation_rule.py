#!/usr/bin/env python3
"""
Thesis Ablation Experiment 2B: Softmax Growth Negotiation Rule Multi-Seed Runner.

Evaluates how temperature-scaled softmax competitive negotiation drives territorial
boundary formation, competitive exclusion, and distinct multi-species domain dynamics.
"""
...
def run_single_negotiation_seed(
    seed: int,
    grid_size: int = 384,
    steps: int = 3600,
    ...
    n_patches: int = 6,
    beta: float = 3.0,
    base_output_dir: str = "results/negotiation_rule"
) -> Dict[str, Any]:
    ...
    print(f"\n=======================================================")
    print(f"=== Softmax Negotiation Rule Ablation (Seed {seed}) ===")
    print(f"=======================================================")
    ...
    params = FlowLeniaParams(
        ...
        beta=beta, v_scale=5.8, alpha_diffusion=0.050
    )
    
    print(f"[Negotiation Rule] Running continuous simulation (3600 steps = 1 min HD, beta={beta})...")
    rng_key, subk = random.split(rng_key)
    ...
    metrics["method"] = "negotiation_rule"
    metrics["mixing_rule"] = "negotiation"
    metrics["beta"] = beta
...
def main():
    parser = argparse.ArgumentParser(description="Softmax Negotiation Rule Ablation Multi-Seed Runner")
    ...
    parser.add_argument("--output_dir", type=str, default="results/negotiation_rule", help="Output directory")
    args = parser.parse_args()
    
    os.makedirs(args.output_dir, exist_ok=True)
    multiseed_summaries = []
    for s in args.seeds:
        summary = run_single_negotiation_seed(
            seed=s,
    ...
    print(f"\n=======================================================")
    print(f"Negotiation Rule completed across all {len(args.seeds)} seeds!")
