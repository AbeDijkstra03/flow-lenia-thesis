...
def create_colosseum_arena_mask(H: int = 384, W: int = 384) -> jnp.ndarray:
    """
    Create a spacious, obstacle-rich oceanic arena with 98.9% open fluid area:
    - 1 Central sleek ring arch (radius 24 to 28 px) with 4 open cardinal archways (width 32 px)
    ...
    gate_ang = 32.0 / 26.0
    is_gate = (
        (np.abs(theta) < gate_ang / 4.0) |
        (np.abs(theta - np.pi / 2.0) < gate_ang / 4.0) |
        (np.abs(theta + np.pi / 2.0) < gate_ang / 4.0) |
        (np.abs(np.abs(theta) - np.pi) < gate_ang / 4.0)
    )
...
def run_single_epic_seed(
    ...
    print(f"Arena Architecture  : Colosseum + 4 Dynamic Chemotactic Sanctuaries (chi = {chemotaxis_chi})")
    print(f"Physics Parameters  : v_scale = 9.0 | alpha = 0.055 | mut_interval = 25")
    print(f"======================================================================")
    
    rng_key = random.PRNGKey(seed)
    wall_mask = create_colosseum_arena_mask(H, W)
    ...
    # Inject 4-sanctuary resource map
    ...
    def _epic_step_fn(curr_st, key, do_mutation):
        mass_primary = curr_st.mass[0] * wall_mask
        ...
        # Compute spatial resource gradient for chemotaxis
        rx, ry = compute_sobel_gradients(new_res)
    ...
    for chunk_idx in range(num_chunks):
        ...
        def _scan_chunk(c, step_i):
            curr_s, k = c
            k, sk = random.split(k)
            do_mut = ((step_cursor + step_i) % mutation_interval) == 0
            nxt = _epic_step_fn(curr_s, sk, do_mut)
        ...
        for f_idx in range(mass_chunk_np.shape[0]):
            ...
            rgb = render_dual_panel_rgb(m_frame, g_frame, n_patches=n_patches, wall_mask=wall_mask)
