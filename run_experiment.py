...
# Supplementary / exploratory experiments (not part of the core thesis narrative)
from experiments.supplementary.run_topological_transport import main as run_topological_main
from experiments.supplementary.run_predator_prey import main as run_predator_prey_main
from experiments.supplementary.run_traveling_salesperson import main as run_tsp_main
from experiments.supplementary.run_collective_bridge import main as run_collective_bridge_main
...
def main():
    ...
    parser.add_argument(
        "--mode", type=str,
        choices=[
            # Core thesis experiments
            "imgep", "wall_obstacle", "chemotaxis_calibration", "barrier_constriction", "depletion",
            "gene_mutation", "negotiation", "agentic_loop", "epic", "scaleup", "orbium", "showcase",
            # Supplementary / exploratory experiments (results/supplementary/)
            "topological_transport", "maze", "predator_prey", "tsp", "traveling_salesperson",
            "collective_bridge", "bridge_building", "bridge",
        ],
        default="imgep",
        help="Experiment execution mode (default: imgep). Supplementary modes write to results/supplementary/."
    )
    parser.add_argument("--scenario", type=str, choices=["all", "maze", "dynamic_reroute", "tokyo_rail", "swarm_channeling"], default="all", help="Scenario for topological_transport mode")
    ...
    elif args.mode in ["topological_transport", "maze"]:
        out_dir = args.output_dir or "results/supplementary/topological_transport"
        sys.argv = [
            sys.argv[0],
            "--scenario", str(args.scenario),
            "--grid_size", str(args.grid_size),
            "--steps", str(args.steps if args.steps >= 2500 else 3600),
            "--sample_interval", str(args.sample_interval if args.sample_interval <= 5 else 3),
            "--seeds"
        ] + seed_strs + ["--output_dir", out_dir]
        run_topological_main()
        
    elif args.mode == "predator_prey":
        out_dir = args.output_dir or "results/supplementary/predator_prey"
        sys.argv = [
            sys.argv[0],
            "--grid_size", str(args.grid_size if args.grid_size >= 256 else 256),
            "--steps", str(args.steps if args.steps >= 2500 else 4500),
            "--sample_interval", str(args.sample_interval if args.sample_interval <= 5 else 3),
            "--seeds"
        ] + seed_strs + ["--output_dir", out_dir]
        run_predator_prey_main()
        
    elif args.mode in ["tsp", "traveling_salesperson"]:
        out_dir = args.output_dir or "results/supplementary/traveling_salesperson"
        sys.argv = [
            sys.argv[0],
            "--grid_size", str(args.grid_size if args.grid_size >= 256 else 256),
            "--steps", str(args.steps if args.steps >= 2500 else 4500),
    ...
    elif args.mode in ["collective_bridge", "bridge_building", "bridge"]:
        out_dir = args.output_dir or "results/supplementary/collective_bridge"
        sys.argv = [
            sys.argv[0],
            "--grid_size", str(args.grid_size if args.grid_size >= 256 else 256),
            "--steps", str(args.steps if args.steps >= 2500 else 4500),
