...
def save_rollout_mp4(
    ...
    for f in range(S):
        ...
        left_img = colorize_multi_species_frame(frame_2d, gid_f, wall_mask=wall_mask)
        
        if dual_panel:
            right_img = render_physical_frame(frame_2d, wall_mask=wall_mask, com_pos=(cy, cx))
...
def extract_trajectory_filmstrip(
    ...
    ideal for LaTeX subfigures and thesis chapters.
    """
    if frames.ndim == 4:
        frames = frames[:, 0, :, :]
    ...
    for p in pcts:
        ...
        left_rgb = colorize_multi_species_frame(frame_2d, gid_f, wall_mask=wall_mask)
        if dual_panel:
            right_rgb = render_physical_frame(frame_2d, wall_mask=wall_mask)
