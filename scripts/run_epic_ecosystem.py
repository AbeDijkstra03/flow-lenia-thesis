...
def run_epic_ecosystem(
    ...
    print("=" * 70)
    print("🎬 EPIC FLOW-LENIA ECOSYSTEM SIMULATION (ZOOMED 256x256) 🎬")
    print("=" * 70)
    ...
    wall_mask = create_arena_pillars_mask(H, W) if with_obstacles else None
    
    # 1. Precompute concentric ring kernels
    rng_key, subk = random.split(rng_key)
    radii = jnp.sort(random.uniform(subk, (K,), minval=6.0, maxval=14.0))
    print("\n[1/4] Precomputing Fourier-domain multi-shell kernels...")
    kernel_ffts = precompute_kernel_ffts(radii, H, W)
    
    # 2. Configure 8 heterogeneous species genomes with rich morphological diversity
    ...
    meta = {
        ...
        "v_scale": v_scale,
        "alpha_diffusion": alpha_diff,
        "beta_negotiation": beta_negotiation,
...
def main():
    ...
    parser.add_argument("--grid_size", type=int, default=256, help="Grid size resolution (default: 256)")
    ...
    parser.add_argument("--no_obstacles", action="store_true", help="Disable obstacle pillars")
    ...
    run_epic_ecosystem(
        ...
        with_obstacles=not args.no_obstacles
    )
